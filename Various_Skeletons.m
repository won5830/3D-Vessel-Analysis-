...
if factor>1
    for i=1:length(b) %각 cell에 대해 들어가며 
        inds=round(1:factor:size(b{i},1)); %행수 1: factor : 각 cell 별 행수 
        b{i}=b{i}(inds,:);
    end
end
...
inds=[0 cumsum(inds)];
p=cell2mat(b);
[v e]=costumVoronoi(p);  %외곽 점들을 다 담은게 p라는 행렬이고 이걸 voronoi diagram의 
...
function [v e]=costumVoronoi(V) %껍데기가 주어졌을 때, voronoi diagram 계산 
...
%  
% Output: v is the matrix of the Voronoi vertices. %이건 할 수 있어...
